...
# 개방현에 대한 **의견 없음**. 개방현을 쓸지는 손 이동·현 유지 비용이 정하고
# 여기서 미리 편을 들지 않는다.
#
# 값을 0.4(기피)에서 0으로 내린 근거. 자리 정답이 영상 8마디 하나였을 때는
# 0.4가 맞았지만, 사람 채보 네 곡(344마디)으로 늘리자 뒤집혔다:
#
#   open   IDMT 948음   실곡 4곡   HTH    Billie Jean   Drowning   예뻤어
#   +0.4      79.8%      69.5%      4%       87%          69%       77%
#    0.0      70.2%      75.3%     73%       83%          69%       69%
#
# 두 정답셋이 반대를 가리킨다. 이 파일이 이미 적어둔 판정 규칙 두 가지를
# 그대로 따랐다 — ①IDMT는 곡당 21초 리프라 한 현에 머무는 경향이 강해
# 실사용을 대표하지 않는다 ②둘 중 **나쁜 쪽**을 기준으로 고른다. 나쁜 쪽이
# 69.5%에서 70.2%로 올라간다.
#
# HTH가 4%였던 이유가 이것 하나다. 정답 45마디 중 39마디가 개방현인데
# (A2 = A현 0프렛), 우리는 같은 음을 E현 5프렛으로 밀어넣었다. 두 후보의
# 비용이 `0.03×5 + 얇은현0` 대 `0 + 개방0.4 + 얇은현0.15`로 갈렸다.
#
# 대신 IDMT 정확도 9.6pp를 내준다. 감춘 손해가 아니라 치른 값이다.
# 재현: python eval/eval_fretting.py + eval/eval_songsterr.py 4곡
W_OPEN_PENALTY = 0.0
