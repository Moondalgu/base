...
#
# **`bassclean.GATE_TARGET_GRID_RATIO`(0.95)와 같은 값이면 안 된다.** 숫자는
# 같아 보여도 **재는 대상이 다르다.**
#
# | | 무엇에 대해 재는가 | 0.95를 적용하면 |
# |---|---|---|
# | `bassclean` 게이트 발동 | IDMT **정답 온셋** | 정상 곡 2/17 제외 (적절) |
# | 이 문턱 (리듬 신뢰) | **우리 검출 온셋** | 실곡 4/4 제외 (전부 불신) |
#
# 우리 검출은 정답보다 흔들리므로 같은 자를 대면 안 된다. 두 값을 같게 두라고
# 적어놨던 것이 이 실수의 원인이다 — "같은 질문"이 아니라 **같은 지표를 다른
# 모집단에 쓴 것**이었다.
#
# **0.85는 실측에서 나온 값이다.** 격자 정렬률과 실제 타현 정확도의 상관이
# 0.861이고, 갈리는 자리가 뚜렷하다:
#
#     Champagne (커버, 게이트 동작)  0.875 -> 타현 63%
#     Queen                          0.777 -> 타현  7%
#     Come Together                  0.751 -> 타현  7%
#     Virtual Insanity               0.674 -> 타현  6%
#
# 0.875와 0.777 사이에서 63%와 7%로 갈린다. 0.85를 그 사이에 둔다.
#
# 표본이 네 곡뿐이라 정밀한 값은 아니다. 골든셋이 늘면 다시 잰다.
# 재는 법: 위 표를 다시 만든다(`eval/eval_songsterr.py` + `eval_video_bars.py`).
TRUSTED_GRID_RATIO = 0.85
